...
- Require the full planning package the prompt specifies, including `README.md`, `system-summary.md`, `spec-map.md`, `domain-glossary.md`, `invariants.md`, `acceptance-matrix.md`, `build-order.md`, `task-graph.md`, `open-questions.md`, `forbidden-shortcuts.md`, `implementation-brief.json`, and `preserved-seams.md`.
...
- The implementation phase must drive the repo toward the spec acceptance gates, especially the foundational runtime and product guarantees:
...
- Verification must attach to the real operator surface and preserved seams, not only to demo flows or fixture-backed substitutes.
- When persisted runtime truth is in scope, the main operator path must create, load, or resume real persisted state.
- Final release checks must cover runtime (`RT-*`), orchestration (`OR-*`), and product (`PX-*`) gates with honest checks.
